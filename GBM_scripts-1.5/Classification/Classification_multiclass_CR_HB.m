...
modelfile = '/home/usuario/Documentos/GBM/Experimentos/ModelosDL/HB_CT_224x224_deco.h5';
...
bw_mask=double(scaled_wsi_SG(:,:,1)>=200);
...
ref=imread('C:\Users\Andres\Desktop\GBM_Project\Experiments\GBM_scripts\FastStain\Sample\ref_CT.jpg');
...
%     tissue_norm = imresize(tissue,1/3);
    ...
    classif_list2(i,1)=double(predictresults(1,1));
...
classif_mask=zeros(size(scaled_wsi_SG));
...
classif_mask2=uint8(classif_mask*255);
...
figure(1), 
subplot(1,2,1), imshow(scaled_wsi,[]); title('H&E');
%subplot(1,3,2), imshow(fuse_grtruth_mask,[]); title('Groundtruth');
subplot(1,2,2), imshow(fuse_classif_mask,[]); title('Classification')
%colorbar('on');
